...
export const test =(req,res)=>{
    res.json({message: 'Api is working',email:'verma@gmail.com'});
};
...
export const updateUser= async (req,res,next)=>{
    ...
    if(req.body.username){
       if(req.body.username.length<5 || req.body.username.length>20){
        return next(errorHandler(400,'Username must be between 5 and 20 character'));
       }
...
export const signout = async(req,res,next)=>{
    try {
        res.clearCookie('access_token').status(200).json('User successfully signed out');
    } catch (error) {
        next(error);
    }
}